...
/* 
XmlWriter takes in enviornment variables and sessionId and creates and writes xml data
Todo: Change to a write stream 
Todo: allow reopening and appending to existing file.
*/
export default class XmlWriter {
  constructor(sessionId, screenWidth, screenHeight) {
    this.data = document.implementation.createDocument(null, 'iTrace_plugin');
    const env = this.data.createElement('environment');
    const root = this.data.getElementsByTagName('iTrace_plugin')[0];
  ...
  /* 
  Input: JSON Object
  Outcome: Appends a new data row to the in memory data
  */
  writeGaze(gaze) {
    const gazes = this.data.getElementsByTagName('gazes')[0];
    const gazeResponse = this.data.createElement('gaze');
    gazeResponse.setAttribute('event_id', gaze.event_id);
    ...
    gazeResponse.setAttribute('token_text', gaze.token_text);
    gazeResponse.setAttribute('token_type', gaze.token_type);
    gazes.appendChild(gazeResponse);
  }

  /*
  Input: filepath string (including filename)
  Output: XML file of gaze data
  */
  ...
  writeToFile(filepath) {
    ...
    fs.writeFile(filepath, format(dataString), err => {
      if (err) throw err;
    });
  }
}
